...
describe("nomination domain service", () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        nominationDomainService = new NominationDomainService(nominationRepositoryMock, domainServiceMock as UserDomainService,
    ...
    test("create a nomination successfully -> returns 201", async () => {
        ...
        (nominationDomainService.nominationRepository.findByEmail as jest.Mock).mockResolvedValue(undefined);
        (nominationDomainService.nominationRepository.createNomination as jest.Mock).mockResolvedValue(nomination);
    ...
    test("create a nomination but user does not exists -> returns 404", async () => {
        ...
        (domainServiceMock.obtainUserByEmail as jest.Mock).mockResolvedValue(undefined);
    ...
    test("create a nomination but already exists by email -> returns 409", async () => {
        ...
        (domainServiceMock.obtainUserByEmail as jest.Mock).mockResolvedValue(user);
        (nominationDomainService.nominationRepository.findByEmail as jest.Mock).mockResolvedValue(nomination);
