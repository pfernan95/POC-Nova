...
describe("user app service", () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        nominationAppService = new NominationAppService(nominationDomainServiceMock as NominationDomainService);
    ...
    test("create a nomination successfully -> returns 201", async () => {
        (nominationAppService.nominationDomainService.createNomination as jest.Mock).mockResolvedValue(response)
    ...
    test("list nominations -> returns 200", async () => {
        ...
        expect(nominationDomainServiceMock.getNominations).toHaveBeenCalledWith(response);
    })
})
