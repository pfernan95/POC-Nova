...
describe("user app service", () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        userAppService = new UserAppService(userDomainServiceMock as UserDomainService);
        request = {
            body: {
                "name": "John",
                "surname": "Doe",
    ...
    test("create an user successfully -> returns 201", async () => {
        (userAppService.userDomainService.createUser as jest.Mock).mockResolvedValue(response)
